...
def parse_args():
    parser = argparse.ArgumentParser(description="Train the coarse 4-point corner detection network.")
    ...
    parser.add_argument("--checkpoint_dir", default="./4PointAllBg", help="Directory to save checkpoints")
...
def build_graph():
    ...
    temp_size = int(h_pool5.get_shape()[1] * h_pool5.get_shape()[2] * h_pool5.get_shape()[3])
    with tf.name_scope("FCLayers"):
        W_fc1 = weight_variable([temp_size, 500], name="W_fc1")
        b_fc1 = bias_variable([500], name="b_fc1")
        h_flat = tf.reshape(h_pool5, [-1, temp_size])
    ...
    with tf.name_scope("loss"):
        ...
        val_summary = tf.summary.scalar('Validate_loss', loss)
    with tf.name_scope("Train"):
        train_step = tf.train.AdamOptimizer(1e-5).minimize(loss)

    return x, y_, y_conv, keep_prob, loss, train_step, train_summary, val_summary
...
def main():
    ...
    print("Train mean subtracted:", np.mean(train_image, axis=(0, 1, 2)))

    x, y_, y_conv, keep_prob, loss, train_step, train_summary, val_summary = build_graph()
    ...
    for i in range(args.steps):
        ...
        if i % 1000 == 0:
            ...
            summary = train_summary.eval(feed_dict={x: train_image[:args.batch_size], y_: train_gt[:args.batch_size], keep_prob: 1.0})
            ...
            val_sum = val_summary.eval(feed_dict={x: validate_image[rand_val], y_: validate_gt[rand_val], keep_prob: 1.0})
        ...
        if i % 500_000 == 0 and i != 0:
            ...
            sess.run(train_step, feed_dict={x: batch, y_: gt, keep_prob: 1.0})
